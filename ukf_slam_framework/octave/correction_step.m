...
for i = 1:m
  ...
  z_points = [sqrt(delta_x.^2 + delta_y.^2);
              atan2(delta_y, delta_x) - sigma_points(3,:)];
  ...
  % Remember to normalize the bearing after computing the difference
  diff_mu = sigma_points - mu;
  diff_mu(3,:) = normalize_angle(diff_mu(3,:));
  Sigma_x_z =  (wc .* diff_mu) * diff_Z'; 

	% TODO: Compute the Kalman gain, line 11 on slide 32
  K = Sigma_x_z * inv(S); % K: nx2
  ...
  mu = mu + K * [z_actual(1) - zm(1); normalize_angle(z_actual(2) - zm(2))];
  sigma = sigma - K * S * K';
